...
<body>
    <div class="card">
        ...
        $query = "SELECT dominio, idWeb FROM webs WHERE idUsuario = $id";
        $result = mysqli_query($conexion, $query);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            if (isset($_GET['boton-eliminar'])) { // Usa isset para verificar si existe
                $idEliminar = intval($_GET['boton-eliminar']); // Sanitizar el ID
                // Realizar la eliminación
                $queryEliminar = "DELETE FROM webs WHERE idWeb = $idEliminar";
                
                if (mysqli_query($conexion, $queryEliminar)) {
                    echo "Web eliminada correctamente.";
                } else {
                    echo "Error al eliminar la web: " . mysqli_error($conexion);
                }
